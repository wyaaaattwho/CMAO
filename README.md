...
Correct-Mode Advantage Optimization 的数学推理实验代码，目前已经进入第二阶段：从“方法有信号”的原型，升级到“能做论文级证据分析和在线 post-training”的实验仓库。
...
当前默认主模型是 `Qwen/Qwen3.5-9B`。Qwen3.5 没有精确 7B 文本模型，9B 是这一代里最接近 7B 档位的 dense 模型。
...
### 2. 用默认配置跑一轮 Math500 评测

默认配置文件在 [configs/experiment/eval_math500_base_qwen35_9b.json](/home/wyatt/research/CMAO/configs/experiment/eval_math500_base_qwen35_9b.json)。
...
python scripts/evaluate.py \
  --config configs/experiment/eval_math500_base_qwen35_9b.json \
  --output-dir outputs/eval/math500_base_qwen35_9b \
  --scoring-config configs/scoring/default.json
```
...
- `advantages.json`
- `report.json`
- `analysis_cases.jsonl` 和 `analysis_summary.json`（仅在 `--analyze-cases` 开启时生成）
...
如果你想手动分步调试，每一步也可以单独运行：

```bash
python scripts/sample.py \
  --config configs/experiment/eval_math500_base_qwen35_9b.json \
  --output outputs/math500_samples.json

python scripts/score.py \
  --input outputs/math500_samples.json \
  --output outputs/math500_scores.json \
  --config configs/scoring/default.json

python scripts/advantage.py \
  --input outputs/math500_scores.json \
  --output outputs/math500_advantages.json \
  --config configs/scoring/default.json

python scripts/rerank_eval.py \
  --input outputs/math500_advantages.json \
  --output outputs/math500_report.json

python scripts/report.py \
  --input outputs/math500_advantages.json \
  --output outputs/math500_report_pretty.json

python scripts/analyze_cases.py \
  --input outputs/math500_advantages.json \
  --output-prefix outputs/math500_analysis
```

也可以用统一 CLI：

```bash
cmao sample --config configs/experiment/eval_math500_base_qwen35_9b.json --output outputs/math500_samples.json
cmao score --input outputs/math500_samples.json --output outputs/math500_scores.json --config configs/scoring/default.json
cmao advantage --input outputs/math500_scores.json --output outputs/math500_advantages.json --config configs/scoring/default.json
cmao rerank_eval --input outputs/math500_advantages.json --output outputs/math500_report.json
cmao report --input outputs/math500_advantages.json --output outputs/math500_report_pretty.json
cmao analyze_cases --input outputs/math500_advantages.json --output-prefix outputs/math500_analysis
```

### `evaluate`

把 `sample`、`score`、`advantage` 和 `report` 串成一个入口。适合正式评测时使用，避免手动管理多个中间命令。

常用参数：

- `--config`
  实验配置，例如 `configs/experiment/eval_math500_grpo_qwen35_9b.json`
- `--output-dir`
  本次评测产物目录
- `--scoring-config`
  打分和 advantage 配置，默认建议使用 `configs/scoring/default.json`
- `--analyze-cases`
  同时导出案例分析
- `--case-output-prefix`
  自定义案例分析输出前缀，默认是 `<output-dir>/analysis`
...
python scripts/train_online_grpo.py \
  --config configs/training/math500_online_grpo_qwen35_9b_lora.json
```

统一 CLI 也支持：

```bash
cmao train_online_grpo --config configs/training/math500_online_grpo_qwen35_9b_lora.json
```
...
- `online_metrics.jsonl`
  每轮训练的 reward、advantage、KL、loss、正确率、截断率等指标
- `online_rollouts.jsonl`
  每轮采样组的摘要和样本预览
...
代码里仍保留 `gsm8k` 加载和判题兼容逻辑，但当前实验配置不再维护 GSM8K 路线。
